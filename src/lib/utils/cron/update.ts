...
async function startCronJob() {
  try {
    const lastGame = await getTotalItemsToRun(itemsToRun);
    ...
    const totalItems = lastGame - (lastItemSaved?.numero ?? 0);

    if (totalItems > 0 && lastItemSaved?.numero) {
      logger.info(`Total items to process: ${totalItems}. Last item saved: ${lastItemSaved?.numero ?? 0}. Last game: ${lastGame}.`);

      currentId = lastItemSaved.numero;
      let isRunning = false;
      ...
      logger.info(`No items to process. Last item saved: ${lastItemSaved?.numero ?? 0}. Last game: ${lastGame}.`);
      process.exit(0);
    }
